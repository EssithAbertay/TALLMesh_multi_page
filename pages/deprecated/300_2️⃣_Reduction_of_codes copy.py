...
# janky solution to keep track of the processing state (for incremental vs auto)
def reset_processing_state():
    """Reset all processing-related session state variables."""
    st.session_state.processing_started = False
    st.session_state.current_index = 0
    st.session_state.reduced_df = None
    st.session_state.total_codes_list = []
    st.session_state.unique_codes_list = []
...
def process_single_file(reduced_df: Optional[pd.DataFrame], file: str, model: str, prompt: str, 
                        model_temperature: float, model_top_p: float, include_quotes: bool) -> Tuple[pd.DataFrame, int, int]:
    """
    Process a single file and compare it with the existing reduced codes.
    
    Args:
        reduced_df (pd.DataFrame or None): The current reduced DataFrame, or None if this is the first file.
    ...
    Returns:
        tuple: A tuple containing the updated reduced DataFrame, total codes count, and unique codes count.
    """
    df = pd.read_csv(file)
    ...
    if reduced_df is None:
        return df, len(df), len(df['code'].unique())
    else:
        new_reduced_df, _, _ = compare_and_reduce_codes(reduced_df, df, model, prompt, model_temperature, model_top_p, include_quotes)
        total_codes = len(new_reduced_df)
        unique_codes = len(new_reduced_df['code'].unique())
        return new_reduced_df, total_codes, unique_codes

def incremental_process_files(selected_project: str, selected_files: List[str], model: str, prompt: str, 
                              model_temperature: float, model_top_p: float, include_quotes: bool):
    """
    Process files incrementally, allowing user interaction between each file comparison.
    """
    if st.session_state.current_index < len(selected_files):
        current_file = selected_files[st.session_state.current_index]
        status_message = st.empty()
        status_message.info(f"Processing file {st.session_state.current_index + 1}/{len(selected_files)}: {os.path.basename(current_file)}")

        with st.spinner("Reducing codes... This may take a moment."):
            st.session_state.reduced_df, total_codes, unique_codes = process_single_file(
                st.session_state.reduced_df, current_file, model, prompt, 
                model_temperature, model_top_p, include_quotes
            )

        st.session_state.total_codes_list.append(total_codes)
        st.session_state.unique_codes_list.append(unique_codes)

        # Display current results
        st.write("Current Reduced Codes:")
        st.dataframe(st.session_state.reduced_df)

        st.write("Code Reduction Results:")
        results_df = pd.DataFrame({
            'file': [os.path.basename(f) for f in selected_files[:st.session_state.current_index + 1]],
            'total_codes': st.session_state.total_codes_list,
            'unique_codes': st.session_state.unique_codes_list
        })
        st.dataframe(results_df)

        status_message.success(f"Processed file {st.session_state.current_index + 1}/{len(selected_files)}: "
                               f"Total codes = {total_codes}, Unique codes = {unique_codes}")

        # Increment the index and add a continue button
        st.session_state.current_index += 1
        if st.session_state.current_index < len(selected_files):
            if st.button("Continue to next file", key=f"continue_{st.session_state.current_index}"):
                st.rerun()
        else:
            st.success("All files processed. You can now save or download the results.")
            
            # Save final results
            save_reduced_codes(selected_project, st.session_state.reduced_df, 'reduced_codes')
            results_path = os.path.join(PROJECTS_DIR, selected_project, 'code_reduction_results.csv')
            results_df.to_csv(results_path, index=False)
            
            # Provide download buttons
            csv = st.session_state.reduced_df.to_csv(index=False).encode('utf-8')
            st.download_button(
...
def main():
    ...
    # Reset processing state when changing projects
    ...
    if selected_project != "Select a project...":
        ...
        # Add a radio button to choose between incremental and full (automatic) processing
        # Add a radio button to choose between incremental and full (automatic) processing
        processing_mode = st.radio(
            "Choose processing mode:",
            ("Automatic Processing", "Incremental Processing"),
            help="Automatic Processing: Process all files at once. Incremental Processing: Review results after each file."
        )

        # Add a "Start Processing" button for both modes
        if st.button("Start Processing"):
            reset_processing_state()
            st.session_state.processing_started = True
            st.rerun()

        # Add a "Reset Processing" button
        if st.button("Reset Processing"):
            reset_processing_state()
            st.rerun()

        # Only process if explicitly started
        if st.session_state.get('processing_started', False):
            if processing_mode == "Automatic Processing":
                st.divider()
                ...
                incremental_process_files(selected_project, selected_files, selected_model, prompt_input, 
                                          model_temperature, model_top_p, include_quotes)
