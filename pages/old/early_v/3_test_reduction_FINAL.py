...
def main():
    ...
    if uploaded_file_1 and uploaded_file_2 and submit_button:
        ...
        if false_values:
           ...
           # false_df = pd.DataFrame([{'code': value.split(':')[0], 'description': value.split(':')[1]} for value in false_values])
            #df1 = pd.concat([df1, false_df], ignore_index=True)
            #df1 = df1.drop(columns=['index'], axis=1)  # Drop the 'index' column
            #df1.index = range(len(df1))  # Set the index to the actual index
           # st.write(df1)
            st.subheader("False Values:")
