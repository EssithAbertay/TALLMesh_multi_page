# -*- coding: utf-8 -*-
"""
Created on Fri Mar  1 15:55:24 2024
    ...
    if uploaded_file_1 and uploaded_file_2 and submit_button:
        ...
        for index, row in df2.iterrows():
            ...
            try:
                ...
                if json_output['value_in_combined_previous']['comparison']== 'false': 
                    ...
                    quote = df2.loc[df2['code'] == row['code'], 'quote'].iloc[0]
                    st.write(quote)
                    quotes.append(quote)
        ...
        if false_values:
            ...
            for value in false_values:
                ...
                quote = df2.loc[df2['code'] == code, 'quote'].iloc[0]
                false_records.append({'code': code, 'description': description, 'quote': quote})
        
            # Create a DataFrame with length matching df1, even if some rows have missing values
            false_df = pd.DataFrame(false_records, index=range(len(df1)))
            df1 = pd.concat([df1, false_df], axis=1)  # Concatenate along columns
            df1 = df1.drop(columns=['index'], axis=1)  # Drop the 'index' column

            st.write(df1)

        # If comparison is true, add the quotes to df1
        if quotes:
            df1['quote'] = quotes
